...
class Seq2SeqTrainer(Trainer):
    def __init__(
        ...
        self.decoder_start_token_id = self.tokenizer.amr_bos_token_id if self.args.task=="text2amr" else self.tokenizer.eos_token_id
        self.decoder_end_token_id = self.tokenizer.amr_eos_token_id if self.args.task=="text2amr" else self.tokenizer.eos_token_id
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if labels is not None:
            if unwrap_model(model)._get_name() in MODEL_FOR_CAUSAL_LM_MAPPING_NAMES.values():
                print("Loss not implemented here!!!")
                # loss = self.label_smoother(outputs, labels, shift_labels=True)
            else:
                # print("Use our label smoothing loss ...")
                lm_logits = outputs[0]
                lprobs = torch.nn.functional.log_softmax(lm_logits, dim=-1)
                loss, nll_loss = label_smoothed_nll_loss(
                    lprobs, labels, self.args.label_smoothing_factor, ignore_index=self.tokenizer.pad_token_id
                )
    ...
    def evaluate(
        ...
        **gen_kwargs
        ...
        gen_kwargs["max_length"] = (
            gen_kwargs["max_length"] if gen_kwargs.get("max_length") is not None else self.args.max_target_length
        )
        gen_kwargs["max_gen_length"] = (
            gen_kwargs["max_gen_length"] if gen_kwargs.get("max_gen_length") is not None else self.args.generation_max_length
        )
    ...
    def prediction_step(
        ...
        max_length = gen_kwargs.pop("max_length")
        max_gen_length = gen_kwargs.pop("max_gen_length")
            ...
            if has_labels:
                if self.label_smoother:
                    ...
                    loss = label_smoothed_nll_loss(
                        lprobs, inputs["labels"], self.args.label_smoothing_factor, ignore_index=self.tokenizer.pad_token_id
                    )[0].mean().detach()
                else:
        ...
        if "labels" in inputs:
            ...
            labels = None
        # print("[xlog] go out predict_step.")
